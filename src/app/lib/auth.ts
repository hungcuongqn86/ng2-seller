...
export class Auth {
    ...
    public createAuthorization(serviceName, serviceRegion, clientId, clientKey,
        ...
        const authScheme = 'WSS';
        const authType = 'wss-request';
        const authAlgorithm = 'WSS-HMAC-SHA256';
        const canonicalUri = this.uriEncode(requestUri, !1);
    ...
    private hmacSha256(key, data) {
        return CryptoJS.HmacSHA256(data, key);
    }

    private yyyyMMdd(timestamp) {
        return timestamp.getUTCFullYear() + this.zeroPad(timestamp.getUTCMonth() + 1) + this.zeroPad(timestamp.getUTCDate());
    }
    ...
    private sha256Hash(data) {
        return this.hex(CryptoJS.SHA256(data));
    }

    public yyyyMMddTHHmmssZ(timestamp) {
        return timestamp.getUTCFullYear() + this.zeroPad(timestamp.getUTCMonth() + 1) + this.zeroPad(timestamp.getUTCDate()) + 'T'
            + this.zeroPad(timestamp.getUTCHours()) + this.zeroPad(timestamp.getUTCMinutes())
